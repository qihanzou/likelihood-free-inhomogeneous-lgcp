...
# L obs
Lobs <- Linhom(X1, r = r, correction = "border")
...
with_progress({
  ...
  res_train <- future_lapply(
    ...
    function(j) {
      ...
      Lfun <- Linhom(X_j, r = r, correction = "border")
...
set.seed(2025)
env_lgcp1(as.numeric(Y_pred_unscaled_M2), "M2")



library(spatstat.geom)
sigma2_hat <- Y_pred_unscaled_M1[1]   
phi_hat    <- Y_pred_unscaled_M1[2]  
log_mu_im <- eval.im(log(lambda_im))  
lambda_marg_im <- eval.im(exp(log_mu_im + 0.5*sigma2_hat))
plot(lambda_marg_im, main = "Marginal fitted intensity", col = viridis::viridis(256))   
plot(X1, add = TRUE, pch = 16, cex = 0.6, col = "black")

sigma2_hat <- fit_clik2$par[1]   
phi_hat    <- fit_clik2$par[2]  
log_mu_im <- eval.im(log(lambda_im))  
lambda_marg_im <- eval.im(exp(log_mu_im + 0.5*sigma2_hat))
plot(lambda_marg_im, main = "Marginal fitted intensity", col = viridis::viridis(256))   
plot(X1, add = TRUE, pch = 16, cex = 0.6, col = "black")
