...
X_all <- unique(unmark(gorillas))


# train and test
n <- npoints(X_all)
idx_keep <- sample(1:n, size = floor(0.8*n), replace = FALSE)
X_train <- X_all[idx_keep]
X_test <- X_all[-idx_keep]
...
fit_kppm <- kppm(X_train ~  elevation + waterdist + slopeangle + heat + slopetype + vegetation, clusters = "LGCP", data = gorillas.extra)
...
#lambda_ppm <- predict(ppm_trend, type = "trend", locations = mean_trend)
#log_lam_im <- eval.im(log(lambda_ppm))
#max(abs(as.vector(lambda_mean_trend$v) - as.vector(lambda_ppm$v)), na.rm = TRUE)


# L obs
# use the pixel image
Lobs <- Linhom(X_train, lambda = lambda_mean_trend, r = r, correction = "border")
Lobs <- as.numeric(Lobs$border - Lobs$r)

# Lobs_ppm <- Linhom(X_train, lambda = lambda_ppm, r = r, correction = "border")
# Lobs_ppm <- as.numeric(Lobs_ppm$border - Lobs_ppm$r)
# Lobs_ppm - Lobs
...
with_progress({
  ...
  res_train <- future_lapply(
    ...
    function(j) {
      ...
      list(iter = j, L = Lvec)  # keep j in the result
    },
    future.seed = seed,
...
#set.seed(seed)
env_lgcp1(as.numeric(Y_pred_unscaled_M1), "DSBI", k)
