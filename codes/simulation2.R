R_address = "C:/Users/qihan/Desktop/DSBI"
setwd(R_address)
set.seed(2025)
...
# ------------------------------------------------------------------------------
ntrain = 10000
beta0_train = runif(ntrain, 5, 5)
beta1_train = runif(ntrain, 0, 0)
...
with_progress({
  p2 <- progressor(along = seq_len(ntrain))
  res_train = future_lapply(
  seq_len(ntrain),
  function(j) {
    p2(sprintf("iteration %d/%d", j, ntrain))
