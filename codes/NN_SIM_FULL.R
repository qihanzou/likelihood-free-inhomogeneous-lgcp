...
# Training:
# ------------------------------------------------------------------------------
plot(beta0, beta0_KPPM)
usr <- c(min(beta0), max(beta0), min(beta0_KPPM), max(beta0_KPPM))
x0  <- max(usr[1], usr[3])
x1  <- min(usr[2], usr[4])
segments(x0, x0, x1, x1, lwd = 2) # black diagonal


# plot(beta1, beta1_KPPM)
# plot(var_test, var_KPPM)
# plot(scale_test, scale_KPPM)
...
error_mu <- estimates_test[, "mu"] - test_par_plot[, "mu"]
error_var <- estimates_test[, "var"] - test_par_plot[, "var"]
error_scale <- estimates_test[, "scale"] - test_par_plot[, "scale"]

error_mu_mincon <- Data_LGCP_test$mu_mincon - test_par_plot[, "mu"]
error_var_mincon <- Data_LGCP_test$var_mincon - test_par_plot[, "var"]
error_scale_mincon <- Data_LGCP_test$scale_mincon - test_par_plot[, "scale"]