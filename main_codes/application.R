...
# envelope test:
env_lgcp1 = function(theta, method){
  sigma2 = theta[1]     
  xi = theta[2]
  env_lgcp <- envelope(
    X1,
    ...
    r = r,
    simulate = expression({rLGCP(model="exponential", mu = log_lam_im - 0.5*sigma2, win = Window(X1), var = sigma2, scale = xi, saveLambda = FALSE)}),
    nsim = 100,
    savefuns = TRUE, global = FALSE, verbose = FALSE
  )
  title_text <- paste0("Envelope of K-function (", method, ")")
...
set.seed(2025)
env_lgcp1(as.numeric(Y_pred_unscaled_DSBI), "DSBI")
